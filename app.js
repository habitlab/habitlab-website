...
(function(){
  var koa, koaStatic, koaRouter, koaLogger, koaBodyparser, koaJsonp, mongodb, getsecret, koaBasicAuth, prelude, kapp, app, auth, ref$, cfy, cfy_node, yfy_node, mongourl, get_mongo_db, get_collection, get_signups, get_uninstalls, get_uninstall_feedback, list_collections, list_log_collections_for_user, list_log_collections_for_logname, get_collection_for_user_and_logname, port, slice$ = [].slice;
  ...
  app.get('/add_uninstall', function*(){
    var ref$, uninstalls, db, err;
    this.type = 'json';
    try {
      ref$ = (yield get_uninstalls()), uninstalls = ref$[0], db = ref$[1];
      (yield function(it){
        return uninstalls.insert(this.request.query, it);
      });
    } catch (e$) {
      err = e$;

...
app.get '/get_user_to_all_install_info', auth, (ctx) ->>
  ctx.type = 'json'
  try
    [installs, db] = await get_installs()
    all_results = await n2p -> installs.find({}).toArray(it)
    ...
    for install_info in all_results
      ...
      if not output[userid]?
        output[userid] = []
      output[userid].push(install_info)
    ctx.body = JSON.stringify(output)
  catch err
    console.log 'error in get_user_to_all_install_info'
    console.log err
    ctx.body = JSON.stringify {response: 'error', error: 'error in get_user_to_all_install_info'}
  finally
    db?close()
...
app.get '/get_user_to_all_uninstall_info', auth, (ctx) ->>
  ctx.type = 'json'
  try
    [uninstalls, db] = await get_uninstalls()
    all_results = await n2p -> uninstalls.find({}).toArray(it)
    ...
    for uninstall_info in all_results
      ...
      if not output[userid]?
        output[userid] = []
      output[userid].push(uninstall_info)
    ctx.body = JSON.stringify(output)
  catch err
    console.log 'error in get_user_to_all_uninstall_info'
    console.log err
...
app.get '/get_user_to_all_uninstall_times', auth, (ctx) ->>
  ...
  try
    ...
    console.log 'error in get_user_to_all_uninstall_times'
    console.log err
    ctx.body = JSON.stringify {response: 'error', error: 'error in get_user_to_all_uninstall_times'}
  finally
    db?close()
